...
const getDotsVisible = paper => {
  let dotsVisible = 0
  paper.forEach(line=> dotsVisible += line.filter(point => point === "#").length)
  return dotsVisible
}

const mySolution = filePath => {
  const {paper, instructions} = parseInputFile(filePath)
  const foldedPaper = applyInstruction(paper, instructions[0])
  // printAsMatrix(foldedPaper)
  const dotsVisible = getDotsVisible(foldedPaper)
  return dotsVisible
}
